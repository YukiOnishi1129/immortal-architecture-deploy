...
Integration TestとE2E Testがどのように実行されるか、フローを詳しく解説します。

---

#### Integration Test の実行フロー

```
┌─────────────────────────────────────────────────────────────────────────┐
│  make test-integration                                                  │
│  go test -tags=integration ./internal/adapter/gateway/db/...            │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  1. SetupPostgres(t) - コンテナ起動                                      │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │  testcontainers-go                                                 │ │
│  │  ┌──────────────────────────────────────────────────────────────┐  │ │
│  │  │  postgres.Run(ctx,                                           │  │ │
...
│  │                              │                                     │ │
│  │                              ▼                                     │ │
│  │  Docker API で PostgreSQL コンテナを起動                            │ │
│  │  待機: "database system is ready to accept connections" × 2回      │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  2. runMigrations() - スキーマ作成                                       │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │  golang-migrate/migrate                                            │ │
│  │  ┌──────────────────────────────────────────────────────────────┐  │ │
│  │  │  migrate.New(                                                │  │ │
│  │  │      "file://../../migrations",   ← SQLファイルの場所         │  │ │
│  │  │      connStr,                     ← コンテナの接続文字列      │  │ │
│  │  │  )                                                           │  │ │
│  │  │  m.Up()                           ← 全マイグレーション実行    │  │ │
│  │  └──────────────────────────────────────────────────────────────┘  │ │
...
│  │                              ▼                                     │ │
│  │  migrations/                                                       │ │
│  │  ├── 000001_create_accounts.up.sql    → CREATE TABLE accounts     │ │
│  │  ├── 000002_create_templates.up.sql   → CREATE TABLE templates    │ │
│  │  ├── 000003_create_fields.up.sql      → CREATE TABLE fields       │ │
│  │  ├── 000004_create_notes.up.sql       → CREATE TABLE notes        │ │
│  │  └── 000005_create_sections.up.sql    → CREATE TABLE sections     │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  3. CreateTestData() - テストデータ投入（Seeding）                       │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │  tests/testutil/fixtures.go                                        │ │
│  │                                                                    │ │
│  │  CreateTestAccount(t, pool, TestAccount{...})                      │ │
│  │      → INSERT INTO accounts (id, email, first_name, ...) VALUES    │ │
│  │                                                                    │ │
│  │  CreateTestTemplate(t, pool, TestTemplate{...})                    │ │
│  │      → INSERT INTO templates (id, name, owner_id) VALUES           │ │
│  │      → INSERT INTO fields (id, template_id, label, ...) VALUES     │ │
│  │                                                                    │ │
│  │  CreateTestNote(t, pool, TestNote{...})                            │ │
│  │      → INSERT INTO notes (id, title, template_id, ...) VALUES      │ │
│  │      → INSERT INTO sections (id, note_id, field_id, ...) VALUES    │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
...
│  │      note, err := repo.Create(ctx, note.Note{...})                 │ │
│  │      assert.NoError(t, err)                                        │ │
│  │  })                                                                │ │
│  │                                                                    │ │
│  │  t.Run("Get", func(t *testing.T) {                                 │ │
│  │      found, err := repo.Get(ctx, noteID)                           │ │
...
└───────────────────────────────┬─────────────────────────────────────────┘
                                ...
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  5. t.Cleanup() - 自動クリーンアップ                                     │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │  テスト終了時に自動実行:                                            │ │
│  │                                                                    │ │
│  │  1. pool.Close()           ← DB接続プールを閉じる                   │ │
│  │  2. pgContainer.Terminate() ← Dockerコンテナを削除                  │ │
│  │                                                                    │ │
│  │  ※ データベースもコンテナと一緒に消える（クリーンな状態）           │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────┘
```
...
└───────────────────────────────┬─────────────────────────────────────────┘
                                ...
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  1. SetupPostgres(t) - コンテナ起動 + マイグレーション                    │
│  （Integration Testと同じ）                                              │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  2. StartTestServer(t, connStr) - APIサーバー起動                        │
│  ┌────────────────────────────────────────────────────────────────────┐ │
...
│  │                                                                    │ │
│  │  // Echoサーバー構築（本番と同じ依存性注入）                         │ │
│  │  e := buildEchoServer(pool)                                        │ │
│  │  ┌──────────────────────────────────────────────────────────────┐  │ │
│  │  │  buildEchoServer(pool):                                      │  │ │
│  │  │                                                              │  │ │
│  │  │  // Repository Factory                                       │  │ │
...
│  │  // → http://127.0.0.1:xxxxx でリクエスト可能                       │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────────────┐
│  3. CreateDefaultTestData(t, pool) - テストデータ投入                    │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │  // E2E用の基本データセット                                         │ │
│  │  data := testutil.CreateDefaultTestData(t, server.Pool())          │ │
│  │                                                                    │ │
│  │  → Account (ID, Email, FirstName, LastName)                        │ │
│  │  → Template (ID, Name, OwnerID, Fields[3])                         │ │
│  │  → Note (ID, Title, TemplateID, OwnerID, Sections[3])              │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
                                ▼
...
│  4. HTTPリクエストでテスト実行                                           │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │  t.Run("POST /api/notes - Create", func(t *testing.T) {            │ │
│  │      resp, _ := http.Post(                                         │ │
│  │          server.URL + "/api/notes",                                │ │
│  │          "application/json",                                       │ │
│  │          strings.NewReader(`{                                      │ │
...
│  │  │  HTTP Request                                                │  │ │
│  │  │       ↓                                                      │  │ │
│  │  │  Echo Router (openapi.RegisterHandlers)                      │  │ │
│  │  │       ↓                                                      │  │ │
│  │  │  NoteController.Create()                                     │  │ │
│  │  │       ↓                                                      │  │ │
│  │  │  NoteInteractor.Create()                                     │  │ │
│  │  │       ↓                                                      │  │ │
│  │  │  NoteRepository.Create() → PostgreSQL (testcontainer)        │  │ │
│  │  │       ↓                                                      │  │ │
│  │  │  NotePresenter → JSON Response                               │  │ │
│  │  └──────────────────────────────────────────────────────────────┘  │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└───────────────────────────────┬─────────────────────────────────────────┘
                                │
...
│  5. t.Cleanup() - 自動クリーンアップ                                     │
│  ┌────────────────────────────────────────────────────────────────────┐ │
│  │  1. server.Close()          ← HTTPテストサーバーを停止              │ │
│  │  2. pool.Close()            ← DB接続プールを閉じる                  │ │
│  │  3. pgContainer.Terminate() ← Dockerコンテナを削除                  │ │
│  └────────────────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│   │   ├── SetupPostgres(t)          # コンテナ起動 + マイグレーション
│   │   ├── NewPool(t)                # pgxpool.Pool 作成
│   │   ├── runMigrations()           # golang-migrate 実行
│   │   ├── getMigrationsPath()       # migrations/ の絶対パス取得
│   │   └── CleanupTables()           # テーブル TRUNCATE（オプション）
│   │
│   └── fixtures.go                   # テストデータ生成
│       ├── CreateTestAccount()       # INSERT accounts
│       ├── CreateTestTemplate()      # INSERT templates + fields
│       ├── CreateTestNote()          # INSERT notes + sections
│       └── CreateDefaultTestData()   # 一括作成（E2E用）
│
└── e2e/                              # E2Eテスト
    ├── testutil/
    │   └── server.go                 # テストサーバー起動
    │       ├── StartTestServer()     # httptest.Server 起動
    │       ├── buildEchoServer()     # 依存性注入済みEcho構築
    │       └── Pool()                # DB接続プール取得
    │
...
2. 本番と同じスキーマ
   ┌────────────────────────────────────────────────────────┐
   │  migrations/ の SQL をそのまま使用                      │
   │  → 本番とテストのスキーマ差異がない                     │
   │  → マイグレーション追加時も自動で反映                   │
   └────────────────────────────────────────────────────────┘

3. t.Cleanup() による自動クリーンアップ
   ┌────────────────────────────────────────────────────────┐
   │  テスト終了時に自動でコンテナ削除                       │
   │  → defer より確実（panicしても実行される）              │
   │  → リソースリークの心配なし                             │
   └────────────────────────────────────────────────────────┘

4. E2Eは本番と同じ依存性注入
   ┌────────────────────────────────────────────────────────┐
   │  buildEchoServer() で本番と同じ構成                     │
   │  → モックなし、実際のRepository使用                     │
   │  → 本当に動作するかを検証                               │
   └────────────────────────────────────────────────────────┘
```
