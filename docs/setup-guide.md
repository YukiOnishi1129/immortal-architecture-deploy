...
### 4-5. Google OAuth 用のシークレット（後で作成）

`GOOGLE_CLIENT_ID` と `GOOGLE_CLIENT_SECRET` は Step 8 で作成します。

### 4-6. 作成したシークレットの確認

作成後、以下の 3 つのシークレットがあることを確認：
...
古い Docker イメージが溜まると料金がかかるため、自動削除ポリシーを設定します。

1. 作成した `cloud-run-repo` をクリック
2. 上部の「クリーンアップポリシー」タブをクリック
3. 「ポリシーを作成」をクリック
4. 以下を設定：
...
| 項目 | 値 |
|-----|-----|
| タグの状態 | `タグ付けされていないイメージのみ` |
| 期間より古い | `7 日` |

5. 「作成」をクリック

> **ポイント**: これにより、タグなしの古いイメージが 7 日後に自動削除されます。`latest` や `SHORT_SHA` でタグ付けされたイメージは保持されます。
...
1. 左メニュー →「トリガー」をクリック
2. 「リポジトリを接続」をクリック
3. 「ソースを選択」で「GitHub (Cloud Build GitHub アプリ)」を選択
4. 「続行」をクリック
5. GitHub の認証画面が表示されたら、アクセスを許可
6. 対象のリポジトリ（Fork した `immortal-architecture-deploy`）を選択
7. 「接続」をクリック

### 6-3. Backend 用トリガーを作成
...
### 6-5. Cloud Build サービスアカウントに権限を付与

Cloud Build がデプロイできるように、権限を設定します。

1. 左メニュー →「IAM と管理」→「IAM」
2. `xxxx@cloudbuild.gserviceaccount.com` という行を探す
   - `xxxx` は数字のプロジェクト番号
...
### 6-6. Secret Manager へのアクセス権限を付与
...
6. 「保存」をクリック
7. 他のシークレット（`CLIENT_ORIGIN`, `BETTER_AUTH_SECRET`）にも同様の権限を付与
...
3. 「代入変数」の `_NEXT_PUBLIC_APP_URL` を Frontend の URL に変更
4. 「保存」をクリック
...
4. 「保存して次へ」をクリック
5. 「スコープ」は何も追加せず「保存して次へ」
6. 「テストユーザー」も何も追加せず「保存して次へ」
7. 「ダッシュボードに戻る」
...
### 8-3. Secret Manager にシークレットを追加

**GOOGLE_CLIENT_ID の作成**
...
|-----|-----|
| 名前 | `GOOGLE_CLIENT_ID` |
| シークレットの値 | メモしたクライアント ID |

4. 「シークレットを作成」をクリック

**GOOGLE_CLIENT_SECRET の作成**
...
| シークレットの値 | メモしたクライアント シークレット |

3. 「シークレットを作成」をクリック

### 8-4. シークレットへのアクセス権限を付与

Step 6-6 と同様に、`GOOGLE_CLIENT_ID` と `GOOGLE_CLIENT_SECRET` にも権限を付与します。
